...
@section('content')
<div x-data="{ showCreate: false, showEdit: false, editData: {}, users: [], editUsers: [], loading: false }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        ...
        <button @click="loading = true; fetch('{{ route('driver.create') }}').then(r=>r.json()).then(d=>{users=d.users; showCreate=true; loading=false})" 
                    ...
                    <tr class="hover:bg-blue-50/40 transition">
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center space-x-1">
                                ...
                                <button @click="loading = true; fetch('{{ route('driver.edit', $d) }}').then(r=>r.json()).then(data=>{editData=data.driver; editUsers=data.users; showEdit=true; loading=false})" class="p-2 text-amber-500 hover:bg-amber-50 rounded-lg transition" title="Edit" :disabled="loading">
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 z-10">
                ...
                <form action="{{ route('driver.store') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="user_id" required class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            <option value="">-- Pilih User --</option>
                            <template x-for="u in users" :key="u.id">
                                <option :value="u.id" x-text="u.name + ' (' + u.email + ')'"></option>
                            </template>
                        </select>
                        <p class="text-xs text-gray-400 mt-1">Hanya user dengan role supir/knek yang tersedia</p>
                        @error('user_id')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Nama <span class="text-red-500">*</span></label>
                            <input type="text" name="nama" required class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">No HP <span class="text-red-500">*</span></label>
                            <input type="text" name="nomor_hp" required class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Alamat <span class="text-red-500">*</span></label>
                        <textarea name="alamat" rows="2" required class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"></textarea>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Role Pekerjaan <span class="text-red-500">*</span></label>
                            <select name="role_pekerjaan" required class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
