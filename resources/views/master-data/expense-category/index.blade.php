...
@section('title', 'Kategori Pengeluaran - LPG Distribution')
...
<div x-data="{ showCreate: false, showEdit: false, editData: {} }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Kelola kategori biaya operasional dan pengeluaran</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6">
        <form method="GET" action="{{ route('expense-category.index') }}" class="flex flex-col sm:flex-row gap-3">
            <div class="flex-1">
                <input type="text" name="search" value="{{ $search }}" placeholder="Cari nama atau jenis kategori..." class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            </div>
            <div class="flex gap-2">
                ...
                <a href="{{ route('expense-category.index') }}" class="px-4 py-2.5 bg-gray-100 text-gray-600 text-sm font-medium rounded-lg hover:bg-gray-200 transition">Reset</a>
            </div>
        </form>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase">Nama Kategori</th>
                        <th class="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase">Jenis Kategori</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-blue-50/40 transition">
                        <td class="px-6 py-4 text-sm text-gray-500">{{ $expenseCategories->firstItem() + $i }}</td>
                        <td class="px-6 py-4 text-sm font-semibold text-gray-800">{{ $cat->nama_kategori }}</td>
                        <td class="px-6 py-4 text-sm text-gray-700">{{ $cat->jenis_kategori }}</td>
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center space-x-1">
                                @can('edit master data')
                                <button @click="editData = { id: {{ $cat->id }}, nama_kategori: '{{ addslashes($cat->nama_kategori) }}', jenis_kategori: '{{ addslashes($cat->jenis_kategori) }}' }; showEdit = true" class="p-2 text-amber-500 hover:bg-amber-50 rounded-lg transition" title="Edit">
                                ...
                                <form action="{{ route('expense-category.destroy', $cat) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus kategori ini?')">
                                    @csrf @method('DELETE')
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-16 text-center">
                            ...
                            <p class="mt-4 text-sm font-medium text-gray-500">Belum ada data kategori pengeluaran</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        <div class="px-6 py-4 border-t border-gray-100">{{ $expenseCategories->withQueryString()->links() }}</div>
        @endif
    </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 z-10">
                <div class="flex items-center justify-between mb-5">
                    <h3 class="text-lg font-bold text-gray-800">Tambah Kategori Pengeluaran</h3>
                ...
                <form action="{{ route('expense-category.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Nama Kategori <span class="text-red-500">*</span></label>
                        <input type="text" name="nama_kategori" required placeholder="Contoh: Solar" class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    <div>
                        ...
                        <input type="text" name="jenis_kategori" required placeholder="Contoh: Operasional" class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    <div class="flex justify-end space-x-3 pt-4 border-t">
                        ...
                        <button type="submit" class="px-6 py-2.5 text-sm font-semibold text-white bg-blue-600 rounded-lg shadow-lg shadow-blue-500/30 transition">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div x-show="showEdit" x-cloak class="fixed inset-0 z-50 overflow-y-auto" x-transition>
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 z-10">
                <div class="flex items-center justify-between mb-5">
                    <h3 class="text-lg font-bold text-gray-800">Edit Kategori Pengeluaran</h3>
                ...
                <form :action="'{{ url('master-data/expense-category') }}/' + editData.id" method="POST" class="space-y-4">
                    @csrf @method('PUT')
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Nama Kategori <span class="text-red-500">*</span></label>
                        <input type="text" name="nama_kategori" x-model="editData.nama_kategori" required class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Jenis Kategori <span class="text-red-500">*</span></label>
                        <input type="text" name="jenis_kategori" x-model="editData.jenis_kategori" required class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>
                    <div class="flex justify-end space-x-3 pt-4 border-t">
                        ...
                        <button type="submit" class="px-6 py-2.5 text-sm font-semibold text-white bg-amber-500 rounded-lg shadow-lg shadow-amber-500/30 transition">Perbarui</button>
                    </div>
                </form>
            </div>
        </div>
